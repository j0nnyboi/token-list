...
import tokenlist from './../tokens/safecoin.tokenlist.json';
...
export class CDNTokenListResolutionStrategy {
  repositories = [
    'https://cdn.jsdelivr.net/gh/j0nnyboi/token-list@main/src/tokens/safecoin.tokenlist.json',
  ];
...
export class SafecoinTokenListResolutionStrategy {
  repositories = ['https://token-list.safecoin.com/safecoin.tokenlist.json'];
  ...
  const responses: TokenList[] = (await Promise.all(
    files.map(async (repo) => {
      try {
        ...
        console.info(
          `@safecoin/token-registry: falling back to static repository.`
        );
        return tokenlist;
      }
    })
